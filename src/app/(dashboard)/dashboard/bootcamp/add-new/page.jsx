...
import { useBootcampManagement } from "@/hooks/useBootcampHooks";
...
export default function AddBootcampPage() {
  ...
  const { createBootcamp, loading, error } = useBootcampManagement();
  ...
  const addPhase = () => {
    if (currentPhase.title && currentPhase.description && currentPhase.duration_weeks) {
      ...
      setCurrentPhase({
        phase_number: prev.phases.length + 2,
        title: "",
        description: "",
        duration_weeks: "",
        learning_objectives: [],
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.success) {
      ...
      toast.error(result.error || "Failed to create bootcamp");
      if (result.errors) {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Add New Bootcamp</h1>
          <p className="text-gray-600 mt-1">
            Create a new intensive bootcamp program
          </p>
        </div>
      </div>
