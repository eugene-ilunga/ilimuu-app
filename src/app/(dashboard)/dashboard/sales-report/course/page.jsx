...
function Dashboard() {
  ...
  const { checkoutData, totalPages, totalItems, loading } = useCourseCheckoutHooks(
    currentPage,
    searchQuery,
    setSelectedOrder,
    paymentStatus,
  ...
  const handleExportToXLSX = async () => {
    ...
    try {
      ...
      const excelData = data.data.map((order, index) => ({
        ...
        "Cours": order.course?.map(c => c.title).join(", ") || "",
      }));
      ...
      XLSX.utils.book_append_sheet(wb, ws, "Course Sales");

      // Generate filename with current date
      const dateStr = new Date().toISOString().split("T")[0];
      const filename = `course-sales-${dateStr}.xlsx`;
            ...
            <table style="width: 100%; border-collapse: collapse; font-size: 11px;">
              <thead>
                <tr style="background: #5F0EB3; color: white;">
                  <th style="padding: 8px; text-align: left; border: 1px solid #d1d5db;">Course</th>
                  <th style="padding: 8px; text-align: right; border: 1px solid #d1d5db;">Price</th>
                </tr>
              </thead>
              <tbody>
                ${data.data.course?.map((item) => `
                  <tr style="border-bottom: 1px solid #e5e7eb;">
                    ...
                    <td style="padding: 8px; text-align: right; font-weight: 600;">$${(item.price || 0).toFixed(2)}</td>
                  </tr>
                `).join("") || ""}
              </tbody>
            </table>
              ...
              <Card>
                <CardHeader className="space-y-4 pb-4">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                      <CardTitle className="text-2xl">Course Purchase</CardTitle>
                      <CardDescription className="mt-1">
                        Manage and view all course purchase orders
                      </CardDescription>
                    </div>
                    <Button
                      variant="outline"
                  ...
                  <>
                    ...
                    <CardContent className="p-6 space-y-6">
                      ...
                      <div className="space-y-3">
                        ...
                        {selectedOrder?.course && selectedOrder.course.length > 0 ? (
                          <div className="space-y-2">
                            {selectedOrder.course.map((c, index) => (
                              <div
                                key={index}
                                onClick={() => route.push(`/course/details?id=${c._id}`)}
                              ...
                              <span className="text-muted-foreground flex items-center gap-1.5">
                                ...
                                <div className="relative group">
                                  ...
                                  <div className="absolute z-10 hidden group-hover:block bg-popover text-popover-foreground text-xs rounded-md p-2 w-48 shadow-lg border -top-2 left-6">
                                    Commission deducted by the organization upon course purchase.
                                  </div>
                                </div>
                              </span>
                            ...
                            <div className="flex items-center justify-between text-sm">
                              <span className="text-muted-foreground flex items-center gap-1.5">
                                ...
                                <div className="relative group">
                                  <HelpCircle className="w-3.5 h-3.5 text-muted-foreground cursor-help" />
                                  <div className="absolute z-10 hidden group-hover:block bg-popover text-popover-foreground text-xs rounded-md p-2 w-48 shadow-lg border -top-2 left-6">
                                    Platform fee charged by the organization for providing the course.
                                  </div>
                                </div>
                              </span>
                              <span className="font-medium text-foreground">
                            ...
                            <div className="flex items-center justify-between text-sm">
                              <span className="text-muted-foreground flex items-center gap-1.5">
                                ...
                                <div className="relative group">
                                  <HelpCircle className="w-3.5 h-3.5 text-muted-foreground cursor-help" />
                                  <div className="absolute z-10 hidden group-hover:block bg-popover text-popover-foreground text-xs rounded-md p-2 w-48 shadow-lg border -top-2 left-6">
                                    Tax applied to the total amount of the course.
                                  </div>
                                </div>
                              </span>
