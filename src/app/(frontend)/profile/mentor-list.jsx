...
import { MessageCircle, DollarSign, CheckCircle, XCircle, Star,RefreshCw } from "lucide-react"
import { ChatDialog } from "@/components/chat-dialog"
import Link from "next/link"


export function MentorList({ enrollments }) {
  ...
  return (
   <>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between gap-6">
                  ...
                  <div className="flex items-center space-x-4 min-w-0 flex-1">
                    <Avatar className="h-16 w-16 ring-2 ring-white shadow-sm flex-shrink-0">
                      <AvatarImage src={enrollment.mentor.image || "/placeholder.svg"} alt={enrollment.mentor.name} />
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        <h3 className="font-semibold text-xl text-gray-900 truncate">{enrollment.mentor.name}</h3>
                        <Badge
                  ...
                  <div className="hidden lg:block min-w-0 flex-1 max-w-xs">
                    ...
                    <div className="space-y-1">
                      ...
                      {enrollment.package.services.length > 3 && (
                        <div className="text-xs text-gray-500 ml-5">+{enrollment.package.services.length - 3} more</div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-6 flex-shrink-0">
                    ...
                    <div className="hidden md:block text-right min-w-0">
                      <div className="space-y-1 mb-3">
                        ...
                        <div className="flex items-center justify-end text-sm">
                          <span className="text-gray-600 mr-2">{active ? "Ends:" : "Ended:"}</span>
                    ...
                    <div className="flex gap-2">
                      {active ? (
                        ...
                        <>
                        <Link href={`/mentor?id=${enrollment.mentor._id}`}>
                          <Button
                ...
                <div className="md:hidden mt-4 pt-4 border-t border-gray-200">
                  <div className="flex justify-between items-center text-sm">
                    ...
                    <div>
                      <span className="text-gray-600">{active ? "Ends: " : "Ended: "}</span>
                      <span className={`font-medium ${active ? "text-green-600" : "text-red-600"}`}>
                        {formatDate(enrollment.endDate)}
                      </span>
                    </div>
                  ...
                  {active && daysRemaining > 0 && (
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-2 mt-2">
                      <p className="text-sm text-blue-800 text-center font-medium">{daysRemaining} days remaining</p>
                    </div>
                  )}
                </div>
                ...
                <div className="lg:hidden mt-4 pt-4 border-t border-gray-200">
                  <h5 className="text-sm font-medium text-gray-700 mb-2">Services included:</h5>
      ...
      <ChatDialog
          ...
          currentUserId={selectedMentor?.user}
          currentUserName={selectedMentor?.mentor?.name}
          currentUserImage={selectedMentor?.mentor?.image || "/placeholder.svg"}
        />
        )}
    </>
  )
