...
export async function POST(req, res) {
  ...
  try {
    ...
    const total = await BookedMentorship.countDocuments({ user: user_id });
    const enrollList = await BookedMentorship.find({ user: user_id })
      .populate({ path: "mentor", select: "_id name image", model: User })
      .populate({
        path: "plan",
