...
import { Star, Clock, ArrowRight, ArrowLeft, Users, Book, BookText } from "lucide-react";
	...
	useEffect(() => {
		const getCourses = async () => {
			try {
				...
				// Filter to show only courses with price === 0
				const freeCourses = courseData.data?.filter(course => 
				...
				<div className="lg:flex justify-between items-center text-left  lg:pt-10 pt-0 pb-10">
					...
					<div className='relative w-full lg:w-1/2  lg:pr-20'>
						...
						<div className="lg:block hidden absolute top-10 right-40">
							<Image className='w-32 h-10' width={40} height={40} alt='' src={textCircle}></Image>
						</div>
						<InfoTitle
								...
								>
									<Link href={`/course/details?id=${course._id}`}>
										...
										<CardContent className="mt-3 pb-2 px-0">
											...
											<div className="flex flex-row flex-wrap justify-between items-center gap-1 px-2 py-1 bg-white rounded-sm shadow-sm w-full mt-2">
											...
											<div className="flex justify-between items-center mt-5">
												...
												<Button className='text-sm font-normal h-8 bg-[--primary] px-3 py-0 rounded-full hover:bg-[--primary]  hover:text-white  text-white '>Enroll <ArrowRight className='text-xs w-5 h-4' /></Button>
											</div>
										</CardContent>
									</Link>
								</Card>
