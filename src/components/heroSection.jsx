...
const HeroSection = () => {
  ...
  const placeholderText = "Search course...";
  ...
  useEffect(() => {
    ...
    if (isTyping) {
      ...
      if (animatedPlaceholder.length < placeholderText.length) {
        timeout = setTimeout(() => {
          setAnimatedPlaceholder(
            placeholderText.substring(0, animatedPlaceholder.length + 1)
          );
        }, 200);
      } else {
      ...
      // Deleting effect - slow
      if (animatedPlaceholder.length > 0) {
        timeout = setTimeout(() => {
          setAnimatedPlaceholder(
            animatedPlaceholder.substring(0, animatedPlaceholder.length - 1)
          );
        }, 100);
      } else {
        // Restart typing
  ...
  }, [animatedPlaceholder, isTyping, searchQuery]);
      ...
      <div className="container mx-auto sm:px-8 lg:px-8">
        ...
        >
          <motion.div
            ...
            variants={fadeInUp}
          >
            <h5 className=" uppercase text-xs lg:text-sm text-[--primary] ">ELIMUU — Apprendre aujourd'hui. Réussir demain.</h5>
            <h1 className="text-2xl sm:text-2xl lg:text-4xl xl:text-5xl font-bold text-gray-900">
              Transformez votre avenir grâce à l'éducation numérique
            </h1>
            <p className="text-base sm:text-lg text-gray-600 mb-6 lg:mt-5 mt-5 ">
              ELIMUU est une plateforme d'apprentissage numérique conçue pour permettre aux étudiants, professionnels et entreprises d'accéder à des formations de qualité, de développer leurs compétences et d'obtenir des certifications reconnues.
            </p>
            ...
            >
              <input
                ...
                type="text"
                placeholder={searchQuery ? "" : animatedPlaceholder || "Search course..."}
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                onKeyDown={handleKeyDown}
                onFocus={() => {
                  if (!searchQuery) setAnimatedPlaceholder("");
                }}
                onBlur={() => {
                  if (!searchQuery) setAnimatedPlaceholder("");
                }}
